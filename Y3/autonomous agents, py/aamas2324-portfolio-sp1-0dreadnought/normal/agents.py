...
class Picker(mesa.Agent):
    ...
    def step(self):
        ...
        if self.wt:
            self.wt = False
        elif self.state != RTRN and (self.storage == 10 or self.life<self.x*self.NUMBER_OF_CELLS):
            self.rtrn()
        else:
            self.move()
    ...
    def move(self):
        if len(self.path)==0:
            if self.state == RTRN:
                ...
                scouts = [A for A in self.model.grid.iter_neighbors((self.x, self.y), False) if A.state == BUSY and isinstance(A,Scout)]
                if len(scouts)>0:
                    self.pick(scouts[0].x,scouts[0].y)
                else:
    ...
    def find_path(self):
        ...
        while True:
            if (0,0) in self.moves(x=newx,y = newy):
                return temp_path
            else:
        ...
        last_tile = (self.x,self.y)
        newx , newy = temp_path[len(temp_path)-1]
        while (0,0) not in self.moves(x=newx,y=newy):
            move = [A for A in self.moves(x=newx,y=newy) if A!=last_tile]
            temp_path.append(move[0])
            last_tile = (newx,newy)
            newx , newy = temp_path[len(temp_path)-1]
    ...
    def pick(self,x,y):
        berry = [A for A in self.model.get_agents_of_type(Bush) if A.x == x and A.y == y]
        berry[0].state = GREN
        self.storage+=1
